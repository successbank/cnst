                        ...
                        <div class="calc-group">
                            ...
                            <select id="lengthSelect" onchange="loadRebarData()" style="font-size: 16px; padding: 12px;">
                                ...
                                <option value="<?php echo $length['length']; ?>" 
                                        data-pieces-per-ton="<?php echo $length['pieces_per_ton'] ?: 0; ?>"
                                        data-total-weight="<?php echo $length['total_weight'] ?: 0; ?>">
                                    <?php echo $length['length']; ?>m (톤당 <?php echo $length['pieces_per_ton'] ?: 0; ?>본)
                                </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div class="calc-result">
                        ...
                        <div class="result-item">
                            ...
                            <div class="price-range-box">
                                ...
                                <div class="price-range-item base-price">
                                    <div class="price-label">기준단가</div>
                                    <div class="price-value"><?php echo number_format($product['price']); ?> <small>원/<?php echo escape($product['unit'] ?: 'TON'); ?></small></div>
                                </div>
                                <div class="price-range-item max-price">
                                    <div class="price-label">최대단가</div>
        ...
        .then(data => {
            if (data.success) {
                ...
                data.data.forEach(item => {
                    ...
                    option.dataset.piecesPerTon = item.pieces_per_ton || 0;
                    option.dataset.totalWeight = item.total_weight || 0;
                    option.textContent = `${item.length}m (톤당 ${item.pieces_per_ton || 0}본)`;
                    lengthSelect.appendChild(option);
                });
                
                lengthSelect.disabled = false;
                document.getElementById('tonQuantity').disabled = false;
            } else {
...
function calculatePrice() {
    ...
    // 총 중량 = DB에 고정된 중량 사용 (있는 경우), 없으면 계산
    const totalWeight = dbTotalWeight > 0 ? dbTotalWeight * tonQuantity : weightPerPiece * actualQuantity;
    
    // 적용 단가 = 기준단가 + 재질 추가단가
    const basePrice = parseFloat(rebarData.base_price) || 0;
    const finalPrice = basePrice + selectedMaterialPrice;
...
function addToQuoteCart() {
    ...
    selectedInfo.length = document.getElementById('lengthSelect').value + 'm';
    selectedInfo.tonQuantity = document.getElementById('tonQuantity').value + '톤';
    
    // actualQuantity가 있으면 사용, 없으면 계산
    const actualQuantityEl = document.getElementById('actualQuantity');
    if (actualQuantityEl && actualQuantityEl.textContent !== '-') {
        selectedInfo.quantity = actualQuantityEl.textContent.replace(/[^0-9]/g, '');
    } else {
        selectedInfo.quantity = '0';
    }
    ...
    if (totalWeightEl && totalWeightEl.textContent !== '-') {
        ...
        selectedInfo.totalWeight = '0kg';
    }
    
    // totalPrice 요소 확인
    const totalPriceEl = document.getElementById('totalPrice');
    if (totalPriceEl && totalPriceEl.textContent !== '-') {
        selectedInfo.totalPrice = totalPriceEl.textContent;
    } else {
        selectedInfo.totalPrice = '0원';
    }
    
    selectedInfo.unitPrice = (document.getElementById('basePrice')?.textContent || '0') + '원/kg';
...
const rebarData = {
<?php else: ?>
// rebarData 재정의 (위에서 let으로 선언됨)
rebarData = {
<?php endif; ?>
    spec_id: <?php echo $rebar_spec['id']; ?>,
    spec_name: '<?php echo $rebar_spec['spec_name']; ?>',
    unit_weight: <?php echo $rebar_spec['unit_weight']; ?>,
    base_price: <?php echo isset($rebar_spec['unit_price']) ? $rebar_spec['unit_price'] : 0; ?> // 관리자 페이지의 현재단가
};
...
document.querySelectorAll('.material-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        if (document.getElementById('resultMaterial')) {
...
function calculateRebarPrice() {
    ...
    // 결과 표시 - 존재하는 요소만 업데이트
    if (document.getElementById('piecesPerTon')) {
        document.getElementById('piecesPerTon').textContent = piecesPerTon.toLocaleString() + '본';
        document.getElementById('piecesPerTonRow').style.display = 'flex';
    }
...
function resetCalculator() {
    ...
    // 결과 표시 초기화 - 존재하는 요소만
    if (document.getElementById('piecesPerTon')) {
        document.getElementById('piecesPerTon').textContent = '-';
        document.getElementById('piecesPerTonRow').style.display = 'none';
    }
