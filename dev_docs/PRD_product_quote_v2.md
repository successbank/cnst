...
## 15. 수량 단위 규칙 정정 (2026-09-11)

초기 구현은 수량 단위를 EA/본/TON/kg/장/SET/M 셀렉트로 제공하고
`price_unit` 과 `calculation_type` 으로 기본값만 골랐다. 이는 기존 화면과 광범위하게 어긋났다.
형강·파이프류 약 470건이 '본' 대신 'EA' 로, 철판류 116건이 '장' 대신 'TON' 으로 표시되었다.

### 15.1 확정 규칙

기존 자동계산 화면(`product_detail_calc.php` 807행/814행)의 판정 기준은 둘뿐이다.

| 조건 | 표시 | 저장값 | 소수 |
|---|---|---|---|
| 계산타입 linear + 카테고리 rebar | `단위 (TON/BD)` | `TON` | 허용 |
| 계산타입 linear + 그 외 | `단위 (본)` | `본` | 불가 |
| 계산타입 sheet 등 | `수량 (장)` | `장` | 불가 |

- 계산타입은 부모 제품을 상속한 값을 쓴다 (`parent_calculation_type ?? calculation_type`).
- `price_unit` 과 `products.unit` 컬럼은 단위 판정에 쓰지 않는다.
  `unit` 컬럼은 914건 중 746건이 비어 있고 `kg/m` 같은 비단위 값이 섞여 있어 사용할 수 없다.

### 15.2 카테고리로 매핑하면 안 되는 이유

`deck-plate` 카테고리에는 계산타입 sheet 제품 1건과 linear 제품 20건이 함께 들어 있다.
카테고리 기준으로 단위를 정하면 둘 중 한쪽이 반드시 깨진다.
판정은 반드시 제품 행의 계산타입으로 해야 한다.

### 15.3 구현

- 신규 `includes/product_unit.php` 의 `productQuantityUnit()` 이 단일 기준이다.
  화면과 서버가 같은 함수를 쓴다.
- 단위 셀렉트를 없애고 제품별 고정 표시로 바꿨다. 기존 화면도 고정 라벨이었다.
- 수량 단위는 사용자 입력이 아니라 제품 속성이므로, `ajax/cart_add.php` 는
  클라이언트가 보낸 `quantity_unit` 을 무시하고 서버에서 다시 판정한다.
- `ajax/cart_update.php` 는 단위 수정을 받지 않는다.
- 본·장 단위는 정수만 허용한다. 철근(톤)만 소수를 허용한다.

### 15.4 검증

전 카테고리 28건(혼재 케이스 `deck-plate` 양쪽, 자식 제품, `price_unit=kg` 제품 포함)에 대해
기존 화면과 신규 화면의 단위를 비교해 **28건 전부 일치**를 확인했다.
단위 위조(`quantity_unit=TON` 직접 전송)는 서버 재판정으로 무시된다.

### 15.5 확인이 필요한 사항

사용자는 "철근은 본"이라고 했으나 기존 화면은 철근을 `TON/BD` 로 표시한다.
본 구현은 "기존 제품들의 단위로 유지" 라는 지시에 따라 기존 화면 표시를 그대로 따랐다.
철근을 `본` 으로 바꾸려면 `includes/product_unit.php` 의 rebar 분기만 수정하면 된다.

---

## 16. 선택 항목 동일화 (2026-09-11)

사용자 보고: `product_detail.php?id=5` 에서 견적요청 방식으로 바꾸면
선택 항목의 단위와 길이가 달라진다.

### 16.1 원인

신규 화면이 기존 화면의 렌더 조건을 따르지 않고 독자적으로 폼을 구성하고 있었다.
전 카테고리를 실제로 렌더해 대조한 결과 12건 이상의 차이가 확인되었다.

| 항목 | 기존 | 신규(수정 전) |
|---|---|---|
| 길이 (판재류 117건) | **항목 없음** | 길이 입력 + M/mm 단위 추가 |
| 길이 (데크플레이트 등) | 항목 없음 | 2.4~12.0m 97개 드롭다운, 6.0m 기본 선택 |
| 길이 (범위 미설정 78건) | 6.0~12.0m 드롭다운 | 직접 입력으로 폴백 |
| 길이 단위 | 선택 UI 없음 (미터 고정) | M/mm 선택 |
| 길이 라벨 | `길이 (미터)` | `길이` |
| 길이 옵션 값 | `8`, `10` | `8.0`, `10.0` |
| 재질 라벨 | `재질 선택` | `재질` |
| 빈 목록 문구 | `선택하세요` | `선택 안 함` |
| 원산지 (자식 제품 30건) | 상속 안 함 | 부모에서 상속 |
| 수량 입력 | `min=1` 정수 | `min=0.001` 소수 |

가장 큰 문제는 판재류였다. 기존 화면은 길이를 선형 제품 조건 안에서만 렌더하므로
판재류에 길이가 없는데, 신규 화면은 조건 없이 항상 렌더했다.
데크플레이트는 길이 범위가 설정되어 있어 사용자가 손대지 않아도
존재하지 않아야 할 길이 6.0m 가 장바구니에 저장되었다.

### 16.2 조치

기존 화면(`product_detail_calc.php`)의 폼 규칙을 그대로 이식했다.

- 길이 항목은 계산타입이 linear 일 때만 렌더한다.
- 길이 드롭다운은 기존과 같은 카테고리 화이트리스트에서만 제공하고,
  범위가 비어 있으면 6.0~12.0m 를 기본값으로 쓴다. 목록 밖 카테고리는 직접 입력이다.
- 길이 단위 선택을 없애고 미터로 고정한다.
- 원산지는 해당 제품의 값만 쓴다. 부모 상속을 하지 않는다.
- 재질은 부모가 있으면 부모 값을 쓴다.
- 라벨과 빈 목록 문구를 기존 문구로 되돌렸다.
- 수량은 전 제품 정수 입력이다. 철근의 소수 입력도 철회했다.
- 서버(`ajax/cart_add.php`)의 허용 목록 판정을 화면과 같은 기준으로 맞췄다.
  기준이 어긋나면 화면에 보이는 값을 골라도 담기가 거부된다.
- 판재류에 길이를 직접 전송해도 서버가 무시한다.

### 16.3 기존 대비 추가되는 항목

견적요청 방식에만 있는 항목은 요청사항 입력란 하나다.
금액 표시와 실시간 계산 결과는 설계상 제외된 것이다.

### 16.4 검증

전 카테고리 28건(혼재 카테고리 양쪽, 자식 제품, 범위 미설정 제품 포함)에 대해
라벨, 원산지·재질·길이 선택지 값과 순서, 기본 선택 위치, 수량 입력 속성을
두 화면에서 추출해 대조했다. **28건 전부 일치**한다.
