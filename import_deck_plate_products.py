...
"""
데크플레이트 제품 임포트 스크립트
파일: /home/successbank/projects/docker/project1/html/114/7/데크플레이트.xlsx
제품 수: 20개
"""
...
# Excel 파일 읽기
excel_file = '/home/successbank/projects/docker/project1/html/114/7/데크플레이트.xlsx'
df = pd.read_excel(excel_file, engine='openpyxl', header=1)

print("=== 데크플레이트 제품 임포트 시작 ===\n")
...
for idx, row in df.iterrows():
    try:
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, 'deck-plate', 'linear', %s, 1)
        """, (product_name, specification, unit_weight, materials_json))

        added_count += 1
        product_id = cursor.lastrowid
        print(f"ADD {added_count:3d}: {spec:25s} | {unit_weight:8.2f} kg/m | ID: {product_id}")
