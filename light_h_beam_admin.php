...
// 총 개수 조회
$count_sql = "SELECT COUNT(*) as total FROM products WHERE category_code = 'light-h-beam'";
$params = [];
if ($search) {
...
// 제품 조회
$sql = "SELECT * FROM products WHERE category_code = 'light-h-beam'";
$params2 = [];
if ($search) {
...
// 모든 재질 목록 가져오기
$materials_sql = "SELECT DISTINCT available_materials FROM products WHERE category_code = 'light-h-beam'";
$materials_stmt = $pdo->query($materials_sql);
$all_materials = [];
while ($row = $materials_stmt->fetch(PDO::FETCH_ASSOC)) {
    if (!empty($row['available_materials'])) {
...
<head>
    ...
    <style>
        * {
        ...
        .weight-status.has-weight {
            background: #d4edda;
            color: #155724;
        }
        .weight-status.no-weight {
            background: #f8d7da;
            color: #721c24;
        }
        .price {
...
<body>
    <div class="container">
        <h1>⚡ 경량H형강(LHB) 제품 관리 시스템</h1>

        <?php
        // 통계 계산
        $stats_sql = "SELECT
            COUNT(*) as total,
            SUM(CASE WHEN specification_weight IS NOT NULL AND specification_weight > 0 THEN 1 ELSE 0 END) as with_weight,
            SUM(CASE WHEN specification_weight IS NULL OR specification_weight = 0 THEN 1 ELSE 0 END) as without_weight
            FROM products
        ...
        <div class="summary-box">
            ...
            <div class="summary-stats">
                <div class="summary-stat">
                    <div class="label">데이터 완료</div>
                    <div class="value" style="color: #4caf50;"><?php echo $stats['with_weight']; ?>개</div>
                </div>
                <div class="summary-stat">
                    <div class="label">데이터 누락</div>
                    <div class="value" style="color: #f44336;"><?php echo $stats['without_weight']; ?>개</div>
                </div>
                <div class="summary-stat">
                    <div class="label">완료율</div>
                    <div class="value" style="color: #2196f3;">
                        <?php echo $stats['total'] > 0 ? round(($stats['with_weight'] / $stats['total']) * 100, 1) : 0; ?>%
                    </div>
                </div>
            </div>
        </div>
            ...
            <table>
                ...
                <tbody>
                    <?php while ($row = $result->fetch(PDO::FETCH_ASSOC)): ?>
                        <?php
                        $materials = !empty($row['available_materials']) ? json_decode($row['available_materials'], true) : [];
                        $origins = !empty($row['available_origins']) ? json_decode($row['available_origins'], true) : [];
                        $has_weight = !empty($row['specification_weight']) && $row['specification_weight'] > 0;
                        ?>
                        <tr>
                            ...
                            <td>
                                <?php if ($has_weight): ?>
                                    <span class="weight"><?php echo number_format($row['specification_weight'], 1); ?> kg/m</span>
                                    <span class="weight-status has-weight">✓</span>
                                <?php else: ?>
                                    <span style="color: #999;">-</span>
                                    <span class="weight-status no-weight">없음</span>
                                <?php endif; ?>
                            </td>
                            <td>
                                <div class="materials">
