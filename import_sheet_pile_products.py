#!/usr/bin/env python3
"""
쉬트파일(강널말뚝) 제품 임포트 스크립트
파일: /home/successbank/projects/docker/project1/html/114/7/쉬트파일.xlsx
제품 수: 22개
"""
...
# Excel 파일 읽기
excel_file = '/home/successbank/projects/docker/project1/html/114/7/쉬트파일.xlsx'
df = pd.read_excel(excel_file, engine='openpyxl', header=1)

print("=== 쉬트파일(강널말뚝) 제품 임포트 시작 ===\n")
...
for idx, row in df.iterrows():
    try:
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, 'sheet-pile', 'linear', %s, 1)
        """, (product_name, specification, unit_weight, materials_json))

        added_count += 1
        product_id = cursor.lastrowid
        print(f"ADD {added_count:3d}: {spec:25s} | {unit_weight:8.1f} kg/m | ID: {product_id}")
