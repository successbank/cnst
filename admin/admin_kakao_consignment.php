...
// 조건 쿼리 생성
$where = ["n.board_type = 'consignment'"];
$params = [];
...
// 데이터 조회
$query = "
    SELECT n.*, c.title as consignment_title, c.company_name, c.category 
    FROM kakao_notifications n
    LEFT JOIN board_consignment c ON n.board_id = c.id
    WHERE $whereClause
...
// 통계
$stats = $kakaoService->getStatistics('consignment', $dateFrom . ' 00:00:00', $dateTo . ' 23:59:59');
?>
        ...
        <div class="kakao-tabs">
            <div class="tab-menu">
                ...
                <a href="admin_kakao_consignment.php" class="tab-item active">위탁판매 알림</a>
        ...
        <div class="filter-section">
            <form method="GET" action="" class="filter-form">
                ...
                <a href="admin_kakao_consignment.php" style="padding: 10px 20px; background: #666; color: white; text-decoration: none; border-radius: 8px; font-size: 14px;">초기화</a>
                <?php endif; ?>
            </form>
        </div>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($notif['consignment_title']): ?>
                                    <a href="../board_view.php?type=consignment&id=<?php echo $notif['board_id']; ?>" target="_blank" style="color: #1A237E;">
                                        <?php echo htmlspecialchars($notif['consignment_title']); ?>
                                    </a>
                                <?php else: ?>
                                    <span style="color: #999;">삭제된 게시물</span>
                                <?php endif; ?>
                            </td>
                            <td>
                                <?php if ($notif['category']): ?>
                                    <span class="category-badge"><?php echo htmlspecialchars($notif['category']); ?></span>
                                <?php endif; ?>
                            </td>
                            <td><?php echo htmlspecialchars($notif['company_name'] ?? '-'); ?></td>
                            <td><?php echo htmlspecialchars($notif['recipient_name']); ?></td>
