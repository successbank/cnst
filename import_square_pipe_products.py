...
"""
사각파이프 제품 임포트 스크립트
파일: /home/successbank/projects/docker/project1/html/114/7/사각파이프.xlsx
제품 수: 94개
"""
...
# Excel 파일 읽기
excel_file = '/home/successbank/projects/docker/project1/html/114/7/사각파이프.xlsx'
df = pd.read_excel(excel_file, engine='openpyxl', header=1)

print("=== 사각파이프 제품 임포트 시작 ===\n")
...
for idx, row in df.iterrows():
    try:
        ...
        # 제품명 생성 (규격의 * 를 × 로 변환)
        product_name = f"사각파이프 {spec.replace('*', '×')}"
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, 'square-pipe', 'linear', %s, 1)
