...
# Read Excel file
excel_file = '/home/successbank/projects/docker/project1/html/114/6/평철.xlsx'
...
for idx, row in df.iterrows():
    ...
    product_name = f"평철 {spec.replace('*', '×')}"
    ...
    try:
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, 'flat-bar', 'linear', %s, 1)
        """, (product_name, specification, unit_weight, materials_json))

        added_count += 1
        print(f"ADD {added_count:3d}: {spec:20s} | {unit_weight:7.3f} kg/m | ID: {cursor.lastrowid}")
