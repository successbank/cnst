-- 충남스틸 웹사이트 데이터베이스 테이블 생성 SQL (MySQL/MariaDB)
...
-- 인덱스 생성
CREATE INDEX idx_board_quote_created ON board_quote(created_at DESC);
CREATE INDEX idx_board_notice_created ON board_notice(created_at DESC);
CREATE INDEX idx_board_news_created ON board_news(created_at DESC);
CREATE INDEX idx_comments_board ON board_comments(board_type, board_id);
